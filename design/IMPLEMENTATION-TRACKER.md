...
| P0 | T474-T544 | 深审计权威、薄协调器与 Action-scoped 成本控制 | While active Action 进入业务执行, when Core 输出下一动作, the product shall 以 fresh Worker 承担业务 Stage，并由无业务历史的 Supervisor 自动驱动 Action 生命周期 | ◐ 2615/1、静态、双宿主 capability/archive 通过；证据、WAIT_RESOURCE、总预算/超调、modelUsage、心跳及参数探测闭环；Claude 成本失败，待同制品双宿主 terminal |
