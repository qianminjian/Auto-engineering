...
| P0 | T407 | 领域事件 Reducer 与 legacy replay | While 新线程推进状态, when Event Stream 重放, the projection shall 由显式领域事件重建且新路径不得写逐 Tick 完整 EngineState；旧流仍可兼容读取 | ✅ 新生产路径按 Channel 生成显式事件且拒绝未映射 Channel；`StateChannelsChanged` 仅保留 legacy replay；304 项第二批矩阵通过 |
...
| P0 | T409 | TickKernel 收敛与旧 façade 绞杀 | While StageHandler 返回 TransitionDecision, when Kernel 应用转换, it shall 不解释 Stage 专属命令式字段，并逐阶段退役旧可变写入路径 | ◐ Stage 命令、完整 state patch 与生命周期 `action_context` 已清零；supplement/refine/convergence/terminal/progress 均已类型化；剩余 `_activate_architecture_plan`、`_offload_stage` 等实现体需迁出 façade 后关闭 |
...
| P0 | T412 | Phase 80 发布收口 | While T404-T411 完成, when 全量、覆盖率、静态、replay、fault injection、archive 和真实产品门禁执行, all required evidence shall 通过后才允许生成下一 RC | ◐ clean full 2243 passed/1 skipped、coverage 90%、Ruff/mypy 通过；20 个既有 SQLite ResourceWarning；待 T409 实现体迁移与真实产品门禁，不生成下一 RC |
