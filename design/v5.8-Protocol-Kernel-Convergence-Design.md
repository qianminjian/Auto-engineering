...
> 状态：历史目标设计，已由 Phase 85 与方案 A 收敛接替｜批准日期：2026-08-09｜对应 Phase：80（T403-T412）
...
- Gate 的 `type=decision|agent_escalation|stage_checkpoint|manual|user`，以及
  `id=state_reconciliation`，统一表示真实人工交互；不得因顶层 `action=gate` 或目标
  `stage` 而降级为 `CONTINUE`。同一判定必须同时用于 Action 编译、宿主投影和 Supervisor。
