...
| P0 | T409 | TickKernel 收敛与旧 façade 绞杀 | While StageHandler 返回 TransitionDecision, when Kernel 应用转换, it shall 不解释 Stage 专属命令式字段，并逐阶段退役旧可变写入路径 | ◐ 四类 Stage 命令与完整 state patch 生产路径已清零；语义游标、进度树、用量采集、快照、checkpoint、offload 已迁至显式 `LifecycleEffects`/`TransitionEffectExecutor` 边界；剩余 supplement、refine、convergence 与 Action 编排待收敛 |
