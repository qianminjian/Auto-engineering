"""P1-A: 3 Agent 类合并为 1 个 BaseAgent(role, system_prompt, llm).

设计动机:
- ArchitectAgent / DeveloperAgent / CriticAgent 三个子类各 ~50-70 行
- 实际逻辑全在 BaseAgent (289 行), 子类仅设默认 system_prompt
- 是过度 OO 拆分 → 合并为 1 个 BaseAgent 类, 3 个 system_prompt 提为 module-level const
...
class TestAgentClassUnification:
    """P1-A: 合并后单 BaseAgent 类支持 3 种 role."""
...
class TestAgentBackwardCompat:
    """旧名 ArchitectAgent/DeveloperAgent/CriticAgent 作为 BaseAgent 的 factory 保留."""

    def test_architect_agent_alias(self) -> None:
        """ArchitectAgent() → BaseAgent(role='architect', system_prompt=ARCHITECT)."""
