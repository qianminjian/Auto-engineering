...
| P0 | T409 | TickKernel 收敛与旧 façade 绞杀 | While StageHandler 返回 TransitionDecision, when Kernel 应用转换, it shall 不解释 Stage 专属命令式字段，并逐阶段退役旧可变写入路径 | ◐ 四类 Stage 命令与完整 state patch 生产路径已清零；语义游标/架构/Critic 生命周期 Effect 已迁至 `TransitionEffectExecutor`；剩余进度树、checkpoint、offload 生命周期待迁出 façade |
