...
| 产品 | 上下文治理 | 循环保险丝 |
|---|---|---|
| Claude Code | 接近窗口时先清旧工具输出，再自动总结并继续 | Action-scoped 进程超时与 thread 总预算约束非交互运行 |
...
当前实现补充：Gate 的完整 message 和文件列表仍属于事实记录；进入下一轮 Action 前由
`project_gate_results` 投影为有界摘要。若仍无法满足 Stage Prompt 硬预算，Core 返回结构化
`ACTION_CONTEXT_TOO_LARGE`，不得以 Python traceback 中断 Tick。
...
| 旧配置 | 处理 |
|---|---|
| `AE_SESSION_MAX_TICKS` | 已移除；Core 不再按 Tick 切换宿主会话 |
| `AE_SESSION_MAX_SECONDS` | 已移除；Core 不再按时长切换宿主会话 |
