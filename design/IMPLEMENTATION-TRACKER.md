...
| P0 | T409 | TickKernel 收敛与旧 façade 绞杀 | While StageHandler 返回 TransitionDecision, when Kernel 应用转换, it shall 不解释 Stage 专属命令式字段，并逐阶段退役旧可变写入路径 | ◐ Stage 命令、完整 state patch 与生命周期 `action_context` 已清零；Architecture 激活和 Stage offload 已迁为独立服务；剩余 Result 投影、Context 构造、Architect 预校验与 Developer Gate 分派需迁出 façade |
