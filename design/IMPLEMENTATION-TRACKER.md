...
| P0 | T474-T530 | 深审计权威、宿主恢复与成本控制 | While active Action 跨会话恢复或小项目进入独立验收, when Core 输出下一动作, the product shall 保持 Canonical Action/Worker journal 权威、只传 digest-bound launcher，并避免固定宿主上下文按回合重放历史 | ◐ T529 将 Worker 4→2、Critic 后直接终态，外层降至 3.79M/3.57M 仍超预算；非翻转优化已耗尽，T530 Action-scoped fresh host context 涉及 D13 翻转，待批准；T526/L4/发布阻断 |
