...
def launcher_prompt(request: ActionExecutionRequest) -> str:
    ...
    return (
        ...
        "ActionContextOutcome。完成时只把 result_contract 声明的业务字段写入 coordinator_result；"
        ...
        "需要 Worker 时，业务产物只能包含 worker_id、status、payload、summary 四类字段，"
        "并原子写入 invocation.outcome_path；"
        "不得写 native_worker_handle、actual_model、"
        "isolation_evidence、attestation、receipt、共享 outcomes 或 Core 字段。原生调用"
        "返回后，Host Driver 必须从 API 取得真实句柄、模型（未知为 unreported）、状态和"
        "隔离证据，交给 Collector 合并；completed 结果若宿主无法提供实际隔离证据，"
        "必须报告失败，不得用 expected 值冒充；启动前失败可明确标注未提供。Worker 不得猜测。共享 outcomes 的 JSON "
        "object，例如：{\"outcomes\":[{\"worker_id\":\"...\",\"native_worker_handle\":"
        "\"native-id\",\"status\":\"completed|failed|cancelled|timed_out\",\"payload\":{},"
        "\"summary\":\"...\",\"actual_model\":\"...\",\"isolation_evidence\":\"...\"}]}；"
        "不得写顶层数组、单个 outcome 或字符串化 JSON。不要手写共享文件；每个 Worker 返回后调用"
        " dev-loop --record-worker-outcome --worker-id <id> --worker-status <status>"
        " --native-worker-handle <handle> --actual-model <model>"
        " --isolation-evidence <evidence> --project-root <root>，"
        "再按 operations.finalize/validate/submit 顺序执行。"
