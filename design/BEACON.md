...
| 32 | **dev-loop 架构重大修正: Agent Tool 模式（v5.1, 2026-07-04）** | 2026-07-04 生产使用报告 (prismscan `_scratch/reports/2026-07-04-dev-loop-execution-analysis.md`) 发现 Python 子进程 (`ae dev-loop "..."`) 模式在 Claude Code agent 内**完全失效**——子进程无法获取 agent 的 ANTHROPIC_AUTH_TOKEN, architect/critic LLM 调用永远失败, 导致 agent 用"串行写代码"替代三阶段流水线. **修正**: v5.1 起 dev-loop 改为 Agent tool 直接执行模式: Plan agent (architect) → Claude Code agent (developer) → code-reviewer agent (critic). 所有角色都在 agent 内用 Agent tool 管理, 复用 agent 的 LLM 连接 (零配置). Python `ae dev-loop` CLI 子进程废弃 (仅保留 `ae doctor/status/checkpoint`). **决策 32 记录修正**: commands/dev-loop.md 全面重写为 236 行 Agent tool spec, CLAUDE.md 新增 "dev-loop Agent Tool 执行模式" 章节. | 2026-07-04 | ✅ |
