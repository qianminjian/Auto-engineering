...
| P0 | T558-T559 | Gate 处置与 rejected journal 同 Action repair | While Core 生成需要人工决策的 Gate 或拒绝已含 Worker outcomes 的 Result, when Action 编译/恢复, the system shall 分别映射 `WAIT_USER` 或原子恢复 journal 权威 outcomes、仅暴露 Coordinator repair、禁止错误执行/重复 spawn/替换事实，并将不可协调冲突投影为 ERROR/Stop Report | ◐ T558/T559 代码与相关回归完成；2692 passed/1 skipped、coverage 90%、Ruff/mypy/sync/metadata/双宿主资产通过；待新 Build 双宿主 L3/L4 验证 |
