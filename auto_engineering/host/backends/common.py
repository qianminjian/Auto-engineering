...
def launcher_prompt(request: ActionExecutionRequest) -> str:
    ...
    return (
        ...
        "需要 Worker 时，每个 Worker 必须先按其 invocation.outcome_path 原子写入自己的"
        " WorkerOutcome；不得写共享 outcomes，Host Collector 会汇总这些已存在文件为 "
        "outcomes 的 JSON object，例如：{\"outcomes\":[{\"worker_id\":\"...\","
