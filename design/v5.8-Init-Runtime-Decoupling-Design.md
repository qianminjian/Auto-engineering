...
> 创建：2026-07-31｜状态：已实施；真实双宿主轨迹待验收｜实施阶段：Phase 73
...
- 项目声明 `browser_e2e` 时，Action 上下文必须附带只读浏览器能力预检；预检可报告系统
  Chrome/Chromium 或 Playwright/Cypress 驱动替代来源，缺失时返回 `BROWSER_RUNTIME_MISSING`，
  不下载驱动、不把未运行 E2E 误报为 Core 失败。
...
宿主依据设计文档和已授权范围执行搭建。成功时提交 `project_setup_completed`；若命令或
工程 Gate 在一次就地修复后仍失败，则提交 `project_setup_failed`，并携带稳定
`failure_code`、有界 `failure_summary` 和 `attempts_in_action`（1 或 2）。Core 不信任
Result 中的文字结论，而是重新运行 Resolver；只有 Profile 校验通过后才进入 gap scan 或
architect。失败 Result 在 Tick 边界进入 EventStore，Core 生成下一 setup Action；连续三次
失败转为可恢复 `resource_wait`。重复 Result 必须幂等，重启后必须恢复同一待处理 Action。
此外，Core 在 init 时记录项目文件基线；Profile 重新解析成功后，Core 必须校验 setup
期间 source/test roots 的新增文件。新增业务源码或业务测试属于
`PROJECT_SETUP_SCOPE_VIOLATION`，即使宿主提交了 `project_setup_completed` 和
`artifacts` 也不得放行。允许范围仅限工程元数据、工具链、源码/测试根和最小非业务
smoke/contract test。
