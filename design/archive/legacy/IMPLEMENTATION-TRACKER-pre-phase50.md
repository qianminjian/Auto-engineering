...
| **50** | **Codex 迁移全面收口（2026-07-27 立项）** | **8** | **5** | **◐ 进行中 — T233-T237 完成；当前 T238 设计资产归档。** |
| **合计** | | | | **Phase 1-49 完成；Phase 50 已完成 5/8，正在实施 T238** |
...
## Phase 50 — Codex 迁移全面收口（2026-07-27）

> 设计：`phase50-codex-migration-closure-design.md`；计划：
> `phase50-codex-migration-closure-PLAN.md`。不修改 Tick 状态机、StageRouter、
> Gate 或 Guardrail 语义；历史资产迁入 `design/archive/` 后保留审计索引。

| 顺序 | T-task | 任务 | 主要产出 | EARS 验收 | 状态 |
|---:|---|---|---|---|:---:|
| 1 | **T233** | CI 与测试真实性 | 修复全仓 Ruff；block detector 只报告、不自动 skip | ✅ 失败历史诊断不再添加 skip marker；新增回归测试；全仓 Ruff 零错误 | ✅ |
| 2 | **T234** | 跨 Agent 规则收口 | 公共模板只保留当前 CLI；同步 AGENTS/CLAUDE | ✅ 规则模板只保留双宿主入口与 resolver；基线引用 pyproject SSOT；10 个同步测试通过 | ✅ |
| 3 | **T235** | Host Adapter 契约收敛 | 检测、能力、事件、CLI、usage source 的统一 Adapter | While 新宿主接入, when 实现 Adapter, the implementation shall 不修改 Core 状态机 | ✅ 19 tests + Ruff + mypy |
| 4 | **T236** | manifest/依赖/配置 SSOT | 中性 metadata、可选 Provider、零死配置 | While 构建 metadata, when 校验配置, declared values shall 与真实消费者及 SSOT 一致 | ✅ 85 tests + metadata + Ruff |
| 5 | **T237** | Release 验收分层 | 自动 smoke 与产品安装验收状态分离 | While 从 Release 安装, when 自动验收完成, the report shall 不冒充真实产品安装 | ✅ JSON 双层状态 + 6 tests + Ruff |
| 6 | **T238** | 设计资产归档 | archive 索引、历史决策迁移、BEACON ≤80 行 | While 读取 BEACON, when 定位当前状态, the reader shall 在 80 行内完成并可追溯历史 | ◐ |
| 7 | **T239** | 当前用户文档重写 | README/Guide/API/培训手册只描述有效能力 | While 检索当前文档, when 搜索退役能力, active sections shall 不宣称其可用 | ☐ |
| 8 | **T240** | 全量验收与收口 | sync/metadata/Ruff/mypy/tests/coverage/Release | While Phase 50 收口, when 全部门禁执行, the repository shall 提供新鲜通过证据 | ☐ |

### Phase 50 完成纪律

- 严格按 T233 → T240 串行推进，同一时刻只允许一个任务为 ◐。
- 功能和缺陷修改执行 Red → Green → Refactor；文档/配置使用契约测试锁定。
- 未经当前用户明确授权，不 commit、不 push、不发布。
