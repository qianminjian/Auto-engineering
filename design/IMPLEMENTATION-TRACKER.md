...
| P0 | T474-T528 | 深审计权威、宿主恢复与成本控制 | While active Action 跨会话恢复或产品宿主连续 Tick, when Core 输出下一动作, the product shall 保持 Canonical Action/Worker journal 权威，并避免宿主历史按回合重复进入模型 input | ◐ T527 journal-authoritative 恢复已通过 2528/1、静态检查和 Codex L3；T526 功能终态通过但外层 input/cache 仍 3.29M/3.05M，成本失败；T528 Action-scoped fresh session 涉及 D13 翻转，待批准 |
